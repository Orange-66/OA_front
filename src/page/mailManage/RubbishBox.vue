<template>
<div class="rubbishBox">
    <b-row>
    <b-col cols="12">
    <CheckBoxTable
    :list_title=rubbishlist_title
    :dataList=rubbishlist
    :fields=fields
    :total=totalDataNum
    ></CheckBoxTable>
      <!-- 查看邮件详细信息 -->
    ...
    <b-row>
        <b-col  cols="2" offset="10">
            <b-button class="btn" variant="primary" @click="toEcharts">查看数据分析</b-button>
        </b-col>
    </b-row>
    <!-- 查看邮件详细信息 -->
        <b-modal id="modal-showDetail" size="lg" title="邮件详情" hide-footer >
...
export default {
   ...
   methods: {
       findbyPage(currentPage){//    分页查找未阅消息
        ...
        this.$http.get(this.baseURL+"getAllByRubbish.action",{pageNum:currentPage,receiveEmailNameId:this.userInfo.userID,pageSize:5})
            .then((data) => {
                if(data.code==1){
                    console.log(data);
       ...
       toDelete(selectidList){//删除邮件
           ...
           this.$http.post(this.baseURL+"deleteRubbishMessage.action",temp)
            .then((data) => {
                if(data.code==1){
       ...
       download(){//下载附件
        document.getElementById("down").href="http://localhost:8082/OA_back_war_exploded/email/downloadFile.action?emailid="+this.emailId
       ...
       toSearch(searchKey){//全文检索
        this.$http.get(this.baseURL+"searchAllTextByRubbish.action",{emailReceiveNameId:this.userInfo.userID,keyWord:searchKey})
            .then((data) => {
                if(data.code==1){
                    console.log(data);
       ...
       toEcharts(){//跳转到邮件数据分析
        this.$router.push({path: "/mailManage/rubbish_echarts",query:{userId:this.userInfo.userID,}});
       }
   },
   created() {
       this.findbyPage(this.currentPageNum);
