<template>
    <div class="unreadMail">
        <CheckBoxTable
        :list_title=unreadMaillist_title
        :dataList=unreadMaillist
        :fields=fields
        :total=totalDataNum
        :showRead=true
        :showDelete=false
...
export default {
   ...
   methods: {
       findbyPage(currentPage){//    分页查找未阅消息
        ...
        this.$http.get(this.baseURL+"getAllByUnRead.action",{pageNum:currentPage,receiveEmailNameId:this.userInfo.userID,pageSize:6})
            .then((data) => {
                console.log(data);
                if(data.code==1){
                    ...
                    for (let j = 0; j < num; j++) {
                        this.unreadMaillist.push({"id":''})
                    }
                    // 插入当前页数据
                    for (let i = 0; i < tempList.length; i++) {
                        this.unreadMaillist.push({
       ...
       getEmailDetail(row){//查看邮件详细信息
           this.$http.get(this.baseURL+"getEmailDetailRead.action",{emailid:row.id})
            .then((data) => {
                if(data.code==1){
                    this.emailId=row.id;
                    var temp=data.data;
       ...
       toSearch(searchKey){//全文检索
        this.$http.get(this.baseURL+"searchAllTextByUnread.action",{emailReceiveNameId:this.userInfo.userID,keyWord:searchKey})
            .then((data) => {
                console.log(data);
                if(data.code==1){
       ...
       toRead(selectidList){//标记为已读
        var temp = new Array();
           for (var i = 0; i < selectidList.length; i++) {
                temp.push(selectidList[i].id);
            }
           this.$http.post(this.baseURL+"toReadMessage.action",temp)
            .then((data) => {
                console.log(data);
                if(data.code==1){
